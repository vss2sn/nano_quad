...
class QuadEncodeMsg : public nodelet::Nodelet
{
 ...
 private:
  ...
  void gains_callback(const quadrotor_msgs::Gains::ConstPtr &msg);
...
void QuadEncodeMsg::gains_callback(const quadrotor_msgs::Gains::ConstPtr &msg)
{
  quadrotor_msgs::Serial serial_msg;
  //serial_msg.header.seq = msg->header.seq;
  serial_msg.channel = channel_;
  serial_msg.type = quadrotor_msgs::Serial::PPR_GAINS;

  quadrotor_msgs::encodePPRGains(*msg, serial_msg.data);
...
void QuadEncodeMsg::onInit(void)
{
  ...
  gains_sub_ = priv_nh.subscribe("gains", 10, &QuadEncodeMsg::gains_callback, this,
                                 ros::TransportHints().tcpNoDelay());
