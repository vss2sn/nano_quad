...
class QuadDecodeMsg : public nodelet::Nodelet
{
 ...
 private:
  ...
  ros::Publisher output_data_pub_, imu_output_pub_, status_pub_, ppr_output_pub_;
...
void QuadDecodeMsg::serial_callback(const quadrotor_msgs::Serial::ConstPtr &msg)
{
  ...
  else if(msg->type == quadrotor_msgs::Serial::PPR_OUTPUT_DATA)
  {
    quadrotor_msgs::PPROutputData::Ptr output_msg(new quadrotor_msgs::PPROutputData);
    if(quadrotor_msgs::decodePPROutputData(msg->data, *output_msg))
    {
      output_msg->header.stamp = msg->header.stamp;
      output_msg->header.frame_id = "/quadrotor";
      ppr_output_pub_.publish(output_msg);
    }
  }
}
...
void QuadDecodeMsg::onInit(void)
{
  ...
  priv_nh.param("quad_type", quad_type, std::string("asctec"));
  ...
  else if(quad_type == "ppr")
  {
    ppr_output_pub_ = priv_nh.advertise<quadrotor_msgs::PPROutputData>("ppr_output", 10);
  }
}
